...
function quickAddToSequencer(mode, presetId) {
  ...
  const newLine = {
    ...
    oscWaveform: config.waveform || 0,
    oscFrequencyHz: config.frequencyHz || 0.5,
    oscEnableRampIn: config.enableRampIn || false,
    oscEnableRampOut: config.enableRampOut || false,
    oscRampInDurationMs: config.rampInDurationMs || 1000,
    oscRampOutDurationMs: config.rampOutDurationMs || 1000,
    chaosCenterPositionMM: config.centerPositionMM || center,
    chaosAmplitudeMM: config.amplitudeMM || 50,
    chaosMaxSpeedLevel: config.maxSpeedLevel || 10,
    chaosCrazinessPercent: config.crazinessPercent || 50,
    chaosDurationSeconds: config.durationSeconds || 30,
    chaosSeed: 0,
